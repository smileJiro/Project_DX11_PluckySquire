...
class CPortalLocker;
class CJellyKing;
class CFatherGame_Progress_PartWing : public CFatherGame_Progress
{
public:
	typedef struct tagFatherGameProgress_PartWing
	{

	}FATHERGAME_PROGRESS_PARTWING_DESC;

private:
	CFatherGame_Progress_PartWing(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext);
	virtual ~CFatherGame_Progress_PartWing() = default;
...
public:
	static CFatherGame_Progress_PartWing* Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext, void* _pArg);
	void					Free() override;
};
