...
//			Section 
// - Section_2D - Narration_2D

//					Section
//				- Section_2D
// - Section_2D_PlayMap - Section_2D_Narration


class CNarration_2D final : public CSection
{
...
private:
	CNarration_2D(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext);
	virtual ~CNarration_2D() = default;
...
private:
	...
	_wstring		m_SectionTextureName = L"";

private:
	HRESULT Ready_Map_2D(SECTION_2D_DESC* _pDesc, _uint _iPriorityKey);

public:
	static CNarration_2D* Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext, _uint _iPriorityKey, SECTION_2D_DESC* _pDesc);
	static CNarration_2D* Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext, _uint _iPriorityKey, json _SectionJson);
	void Free() override;
};
