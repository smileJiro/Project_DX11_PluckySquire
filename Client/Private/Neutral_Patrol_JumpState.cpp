...
CNeutral_Patrol_JumpState::CNeutral_Patrol_JumpState()
{
}

HRESULT CNeutral_Patrol_JumpState::Initialize(void* _pArg)
{
...
void CNeutral_Patrol_JumpState::Set_Bound(_float3& _vPosition)
{
	//일단 현재 위치 기준으로 잡음
...
void CNeutral_Patrol_JumpState::State_Enter()
{
	m_fAccTime = 0.f;
}

void CNeutral_Patrol_JumpState::State_Update(_float _fTimeDelta)
{
	if (nullptr == m_pOwner)
		return;
...
void CNeutral_Patrol_JumpState::State_Exit()
{
	m_fAccTime = 0.f;
	m_fAccDistance = 0.f;
	m_fMoveDistance = 0.f;
...
void CNeutral_Patrol_JumpState::PatrolMove(_float _fTimeDelta, _int _iDir)
{
...
void CNeutral_Patrol_JumpState::Determine_Direction()
{
...
_vector CNeutral_Patrol_JumpState::Set_PatrolDirection(_int _iDir)
{
	_vector vDir = {};
	if (COORDINATE_3D == m_pOwner->Get_CurCoord())
	{
...
void CNeutral_Patrol_JumpState::Check_Bound(_float _fTimeDelta)
{
	_float3 vPos;
	_bool isOut = false;
...
CNeutral_Patrol_JumpState* CNeutral_Patrol_JumpState::Create(void* _pArg)
{
	CNeutral_Patrol_JumpState* pInstance = new CNeutral_Patrol_JumpState();

	if (FAILED(pInstance->Initialize(_pArg)))
	{
		MSG_BOX("Failed to Created : CNeutral_Patrol_JumpState");
...
void CNeutral_Patrol_JumpState::Free()
{
	__super::Free();
}
