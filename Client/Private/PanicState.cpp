...
CPanicState::CPanicState()
{
}

HRESULT CPanicState::Initialize(void* _pArg)
{
...
void CPanicState::Set_Bound(_float3& _vPosition)
{
	//일단 현재 위치 기준으로 잡음
...
void CPanicState::State_Enter()
{
	m_fAccTime = 0.f;
	m_isTurn = true;
}

void CPanicState::State_Update(_float _fTimeDelta)
{
	if (nullptr == m_pOwner)
		return;
	...
	//이동
	if(true == m_isBlock)
		RandomMove(_fTimeDelta, m_iDir);
	else
		PanicMove(_fTimeDelta);
}

void CPanicState::State_Exit()
{
	m_fAccTime = 0.f;
	m_fAccDistance = 0.f;
	m_fMoveDistance = 0.f;
...
void CPanicState::PanicMove(_float _fTimeDelta)
{
	//반대 방향으로 이동
	_vector vDir = m_pOwner->Get_FinalPosition() - XMLoadFloat3(&m_vEvadeTargetPos);
...
void CPanicState::RandomMove(_float _fTimeDelta, _int _iDir)
{
	if (COORDINATE_3D == m_pOwner->Get_CurCoord())
	{
		if (0 > _iDir || 7 < _iDir)
		...
		_vector vDir = XMVector3Normalize(Set_Direction(_iDir));
	...
	else if (COORDINATE_2D == m_pOwner->Get_CurCoord())
	{
		...
		if (true == m_isMove)
		{
			//m_pOwner->Get_ControllerTransform()->Go_Direction(Set_Direction(_iDir), _fTimeDelta);
...
void CPanicState::Determine_Direction(_float _fTimeDelta)
{
	...
	//	//if (false == m_pOwner->Check_InAir_Next(Set_Direction(m_iDir) * m_pOwner->Get_ControllerTransform()->Get_SpeedPerSec(), _fTimeDelta))
	//	//{
	//	//	m_iPrevDir = m_iDir;
...
_vector CPanicState::Set_Direction(_int _iDir)
{
	_vector vDir = {};
	if (COORDINATE_3D == m_pOwner->Get_CurCoord())
	{
...
void CPanicState::Check_Bound(_float _fTimeDelta)
{
	////_float3 vPos;
	...
	////XMStoreFloat3(&vPos, m_pOwner->Get_FinalPosition() + Set_Direction(m_iDir) * m_pOwner->Get_ControllerTransform()->Get_SpeedPerSec() * _fTimeDelta);
	//////나갔을 때 반대방향으로
	////if (COORDINATE_3D == m_pOwner->Get_CurCoord())
	////{
...
CPanicState* CPanicState::Create(void* _pArg)
{
	CPanicState* pInstance = new CPanicState();

	if (FAILED(pInstance->Initialize(_pArg)))
	{
		MSG_BOX("Failed to Created : CPanicState");
