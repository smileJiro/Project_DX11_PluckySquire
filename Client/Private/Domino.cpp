...
void CDomino::OnContact_Enter(const COLL_INFO& _My, const COLL_INFO& _Other, const vector<PxContactPairPoint>& _ContactPointDatas)
{
	...
	m_pGameInstance->Start_SFX_Distance(_wstring(L"A_sfx_domino_hit_domino-") + to_wstring(rand() % 13), Get_FinalPosition(), g_SFXVolume, 0.f, 20.f, 0.7f);
	if (OBJECT_GROUP::INTERACTION_OBEJCT & _Other.pActorUserData->iObjectGroup)
	{
		...
		if(pDIce)
			m_pGameInstance->Start_SFX_Distance(_wstring(L"A_sfx_dice_hit_domino-") + to_wstring(rand() % 13), Get_FinalPosition(), g_SFXVolume, 0.f, 20.f, 0.7f);
	}
}
