...
HRESULT CLevel_Camera_Tool::Ready_Layer_Camera(const _wstring& _strLayerTag, CGameObject* _pTarget)
{
	...
	// Target Camera
	CCamera_Target_Tool_Tool::CAMERA_TARGET_DESC TargetDesc{};

	TargetDesc.fSmoothSpeed = 5.f;
	TargetDesc.eCameraMode = CCamera_Target_Tool::DEFAULT;
...
void CLevel_Camera_Tool::Create_Arms()
{
	...
	CCamera_Target_Tool* pTarget = dynamic_cast<CCamera_Target_Tool*>(CCamera_Manager_Tool::GetInstance()->Get_Camera(CCamera_Manager_Tool::TARGET));
...
void CLevel_Camera_Tool::Change_ArmLength()
{
	...
	if (ImGui::Checkbox("Not LookAt", &m_isNotLookAt)) {
		...
		static_cast<CCamera_Target_Tool*>(pTarget)->Set_LookAt(!m_isNotLookAt);
	}
...
void CLevel_Camera_Tool::Set_MovementInfo()
{
	...
	if (m_ArmNames.size() > 0) {
		...
		if (ImGui::Button("Play Movement")) {
			CCamera_Manager_Tool::GetInstance()->Change_CameraMode(CCamera_Target_Tool::MOVE_TO_NEXTARM);
