...
{
    __super::Action_State_Lock(_fTimeDelta);

    if (STATE_OPEN == m_eCurState)
        return;

    /* 1. 특정 레이어에 대한 검사를 수행한다. */
    const list<CGameObject*>& pObjects = m_pCountingLayer->Get_GameObjects();
    /* 2. 레이어를 확이하며, 현재 Active가 True 이며, Dead 상태가 아닌 오브젝트를 검사한다. */
