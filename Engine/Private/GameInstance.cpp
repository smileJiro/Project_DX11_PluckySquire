...
_bool CGameInstance::MultiSweep(PxGeometry* pxGeom, const _float3& _vOrigin, const _float3& _vRayDir, _float _fDistance, list<CActorObject*>& _OutActors, list<RAYCASTHIT>& _OutRaycastHits)
{
	if (nullptr == m_pPhysx_Manager)
		assert(nullptr);

	return m_pPhysx_Manager->MultiSweep(pxGeom, _vOrigin, _vRayDir, _fDistance, _OutActors, _OutRaycastHits);
}
