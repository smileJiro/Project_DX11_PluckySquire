...
CVIBuffer_Rect_Vertical::CVIBuffer_Rect_Vertical(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    : CVIBuffer(_pDevice, _pContext)
{
}

CVIBuffer_Rect_Vertical::CVIBuffer_Rect_Vertical(const CVIBuffer_Rect_Vertical& _Prototype)
    : CVIBuffer(_Prototype)
{
}

HRESULT CVIBuffer_Rect_Vertical::Initialize_Prototype()
{
...
HRESULT CVIBuffer_Rect_Vertical::Initialize(void* _pArg)
{
    return S_OK;
}

CVIBuffer_Rect_Vertical* CVIBuffer_Rect_Vertical::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
{
	CVIBuffer_Rect_Vertical* pInstance = new CVIBuffer_Rect_Vertical(_pDevice, _pContext);

	if (FAILED(pInstance->Initialize_Prototype()))
	{
		MSG_BOX("Failed to Created : CVIBuffer_Rect_Vertical");
...
CComponent* CVIBuffer_Rect_Vertical::Clone(void* _pArg)
{
	CVIBuffer_Rect_Vertical* pInstance = new CVIBuffer_Rect_Vertical(*this);

	if (FAILED(pInstance->Initialize(_pArg)))
	{
		MSG_BOX("Failed to Cloned : CVIBuffer_Rect_Vertical");
		Safe_Release(pInstance);
	}
...
void CVIBuffer_Rect_Vertical::Free()
{
	__super::Free();
}
