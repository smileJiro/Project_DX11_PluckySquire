...
CRenderGroup_Trail::CRenderGroup_Trail(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext)
    :CRenderGroup_MRT(_pDevice, _pContext)
{
}

HRESULT CRenderGroup_Trail::Initialize(void* _pArg)
{
...
HRESULT CRenderGroup_Trail::Render(CShader* _pRTShader, CVIBuffer_Rect* _pRTBuffer)
{
    ...
    for (auto& pGroupObject : m_GroupObjects)
    {
        if (nullptr != pGroupObject && true == pGroupObject->Is_Render())
            static_cast<CModelObject*>(pGroupObject)->Render_Trail();
...
CRenderGroup_Trail* CRenderGroup_Trail::Create(ID3D11Device* _pDevice, ID3D11DeviceContext* _pContext, void* _pDesc)
{
    CRenderGroup_Trail* pInstance = new CRenderGroup_Trail(_pDevice, _pContext);
    if (FAILED(pInstance->Initialize(_pDesc)))
    {
        MSG_BOX("Failed Create CRenderGroup_Trail");
