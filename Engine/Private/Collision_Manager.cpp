...
HRESULT CCollision_Manager::Unregister_Section(const _wstring& _strSectionKey)
{
    ...
    {
        for (auto pObjvec : (*iter).second)
        {
            for (auto pColl : pObjvec)
            {
                Safe_Release(pColl);
            }
        }
...
{
    // 이미 비트연산자 전용 데이터야
    _uint iRow = _iGroupIDLeft;
    _uint iCol = _iGroupIDRight;
...
void CCollision_Manager::Erase_GroupFilter(_uint _iGroupFilterLeft, _uint _iGroupFilterRight)
{
    // 이미 비트연산자 전용 데이터야
    _uint iRow = (_uint)_iGroupFilterLeft;
    _uint iCol = (_uint)_iGroupFilterRight;

    if (iCol < iRow)
    {
        // 더 큰 GroupID가 열에 와야한다.
        iRow = (_uint)_iGroupFilterRight;
        iCol = (_uint)_iGroupFilterLeft;
    }
...
HRESULT CCollision_Manager::Add_Collider(const _wstring& _strSectionKey, _uint _iGroupFilter, CCollider* _pCollider)
{
    array<vector<CCollider*>, MAX_GROUPID>* pColliders = Find_Colliders(_strSectionKey);
    if (nullptr == pColliders)
        return E_FAIL;

    _uint iGroupIndex = Convert_GroupID_ToIndex(_iGroupFilter);
    if (MAX_GROUPID <= iGroupIndex)
        return E_FAIL;
	_pCollider->Set_CollisionGroupID(_iGroupFilter);
