...
void CEffect_Module::Update_Translations(_float _fTimeDelta, _float4* _pPosition, _float3* _pVelocity, _float3* _pAcceleration)
{
    if (false == m_isActive)
        return;
...
HRESULT CEffect_Module::Add_Float3(const _char* _szTag, const json& _jsonInfo)
{
...
CEffect_Module* CEffect_Module::Create(const json& _jsonModuleInfo)
{
    CEffect_Module* pInstance = new CEffect_Module();

    if (FAILED(pInstance->Initialize(_jsonModuleInfo)))
    {
        MSG_BOX("Failed to Created : CEffect_Module");
...
HRESULT CEffect_Module::Initialize(MODULE_TYPE eType, const _string& _strTypeName)
{
    m_eModuleType = eType;
...
CEffect_Module* CEffect_Module::Create(MODULE_TYPE eType , const _string& _strTypeName)
{
    CEffect_Module* pInstance = new CEffect_Module();
