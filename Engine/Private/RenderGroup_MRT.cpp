...
HRESULT CRenderGroup_MRT::Render(CShader* _pRTShader, CVIBuffer_Rect* _pRTBuffer)
{
...
{
    CRenderGroup_MRT* pInstance = new CRenderGroup_MRT(_pDevice, _pContext);
    if (FAILED(pInstance->Initialize(_pDesc)))
    {
        MSG_BOX("Failed Create CRenderGroup_MRT");
