...
void CCameraArm::Turn_ArmX()
{
    ...
    if (0 < XMVectorGetY(vCrossX))
        RotationMatrix = XMMatrixRotationAxis(vCrossX, m_vRotation.x);
    else
        RotationMatrix = XMMatrixRotationAxis(vCrossX, -m_vRotation.x);
...
void CCameraArm::Turn_ArmY()
{
    // Y축
    _matrix	RotationMatrix = XMMatrixRotationAxis(XMVectorSet(0.f, 1.f, 0.f, 0.f), m_vRotation.y);
