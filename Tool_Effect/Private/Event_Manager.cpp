...
HRESULT CEvent_Manager::Excute(const EVENT& _tEvent)
{
	...
	{
	case EffectTool::EVENT_TYPE::CREATE_OBJECT:
	{
		Excute_CreateObject(_tEvent);
	}
		break;
	case EffectTool::EVENT_TYPE::DELETE_OBJECT:
	{
	...
	case EffectTool::EVENT_TYPE::LEVEL_CHANGE:
	{
		...
		break;
	case EffectTool::EVENT_TYPE::SET_ACTIVE:
	{
...
HRESULT CEvent_Manager::Excute_LevelChange(const EVENT& _tEvent)
{
	...
	{
	case EffectTool::LEVEL_LOADING:
		pChangeLevel = CLevel_EffectLoading::Create(m_pDevice, m_pContext, (LEVEL_ID)iNextChangeLevelID);
		break;
	case EffectTool::LEVEL_TOOL:
		pChangeLevel = CLevel_EffectTool::Create(m_pDevice, m_pContext);
		break;
	default:
		break;
	}
