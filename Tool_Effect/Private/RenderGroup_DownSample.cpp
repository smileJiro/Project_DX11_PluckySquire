...
HRESULT CRenderGroup_DownSample::Initialize(void* _pArg)
{
	...
	RGMRT_BLURDOWN_DESC* pDesc = static_cast<RGMRT_BLURDOWN_DESC*>(_pArg);
...
HRESULT CRenderGroup_DownSample::Render(CShader* _pRTShader, CVIBuffer_Rect* _pRTBuffer)
{
    ...
    if (FAILED(m_pGameInstance->Begin_MRT(TEXT("MRT_DownSample2"), m_pDSV1, true)))
        return E_FAIL;
    ...
    /*if (FAILED(m_pGameInstance->Begin_MRT(TEXT("MRT_DownSample3"), m_pDSV2, true)))
        return E_FAIL;

    Setup_Viewport(_float2(g_iWinSizeX / 64.f, g_iWinSizeY / 64.f));

    vTexelSize = { 16.f / g_iWinSizeX, 16.f / g_iWinSizeY };
    _pRTShader->Bind_RawValue("g_TexelSize", &vTexelSize, sizeof(_float2));
