...
HRESULT CRenderGroup_DirectLights::Render(CShader* _pRTShader, CVIBuffer_Rect* _pRTBuffer)
{
    ...
    _float4x4 matViewInv = m_pGameInstance->Get_TransformInverseFloat4x4(CPipeLine::D3DTS_VIEW);
    _float4x4 matProjInv = m_pGameInstance->Get_TransformInverseFloat4x4(CPipeLine::D3DTS_PROJ);

    /* 3. 조명연산을 위해 픽셀의 좌표를 월드까지 내리는 과정에서 필요한 데이터 및 카메라의 위치 벡터 */
    _pRTShader->Bind_Matrix("g_ViewMatrixInv", &matViewInv);
    _pRTShader->Bind_Matrix("g_ProjMatrixInv", &matProjInv);
